...
class imdb(object):
  ...
  def append_rotate_images(self):
    num_images = self.num_images
    widths = self._get_widths()
    heigths = self._get_heights()
    for i in range(num_images):
        boxes = self.roidb[i]['boxes'].copy()
        oldx1 = boxes[:, 0].copy()
        oldx2 = boxes[:, 2].copy()
    ...
    for i in range(num_images):
        ...
        oldy2 = boxes[:, 3].copy()
        boxes[:, 0] = widths[i] - oldx2 - 1
        boxes[:, 1] = heigths[i] - oldy2 - 1
        boxes[:, 2] = widths[i] - oldx1 - 1
        boxes[:, 3] = heigths[i] - oldy1 - 1
        assert (boxes[:, 2] >= boxes[:, 0]).all()
    ...
    for i in range(num_images):
        ...
        oldy1 = boxes[:, 1].copy()
        oldy2 = boxes[:, 3].copy()
        boxes[:, 0] = heigths[i] - oldy2 - 1
        boxes[:, 1] = oldx1
        boxes[:, 2] = heigths[i] - oldy1 - 1
        boxes[:, 3] = oldx2
        assert (boxes[:, 2] >= boxes[:, 0]).all()
        entry = {'boxes': boxes,
                 'gt_overlaps': self.roidb[i]['gt_overlaps'],
