...
from nets.OICR import bbox_transform, clip_tiled_boxes
...
def im_detect(net, im, roidb_i):
  ...
  for index, scale in enumerate(im_scales):
      ...
      #if index % 2 == 1:
      #    im_blob_input = im_blob.copy()[:, ::-1, :]
      #    ss_boxes  = get_flipper_boxes(ss_boxes.copy(), im_blob.shape[1])
      #else:
      #    im_blob_input = im_blob
      #blobs_scale['im_info'] = np.array([im_blob.shape[1], im_blob.shape[2], scale], dtype=np.float32)
      im_blob = im_blob[np.newaxis, :,:,:]
      img_info = np.array([im_blob.shape[1], im_blob.shape[2], scale], dtype=np.float32)
      bbox_pred, rois ,det_cls_prob , det_cls_prob_product, refine_prob_1, refine_prob_2, refine_prob_3, rotate_score = net.test_image(im_blob, img_info, ss_boxes)
      boxes = rois[:, 1:5] / scale
      #refine_score = (refine_prob_1+refine_prob_2 + refine_prob_3)/3
      scores = np.reshape((refine_prob_1+refine_prob_2 + refine_prob_3 + rotate_score)/4, [det_cls_prob_product.shape[0], -1])
      #scores = np.reshape((refine_score + rotate_score) / 2, [det_cls_prob_product.shape[0], -1])
      ...
      if cfg.TEST.BBOX_REG:
          ...
          pred_boxes = bbox_transform(boxes, box_deltas, cfg.TRAIN.BBOX_REG_WEIGHTS)
          pred_boxes = clip_tiled_boxes(pred_boxes, im.shape)
  ...
  #print('score ',np.array(score_list).shape)
  #print('pred_boxes  ',np.array(pred_box_list).shape)
  #print('det_cls_prob  ',np.array(det_cls_prob).shape)
  #print('mean score 0', (np.array(score_list).mean(0).shape))
  #print('mean score 1', (np.array(score_list).mean(1).shape))
  #print('mean score 2', (np.array(score_list).mean(2).shape))
  ...
  #_, _ , bbox_pred, rois ,det_cls_prob , _ , det_cls_prob_product = net.test_image(blobs['data'], blobs['im_info'], ss_boxes)
  
  #boxes = rois[:, 1:5] / im_scales[0]
  #scores = np.reshape(det_cls_prob_product, [det_cls_prob_product.shape[0], -1])
  #bbox_pred = np.reshape(bbox_pred, [bbox_pred.shape[0], -1])
  #if cfg.TEST.BBOX_REG:
